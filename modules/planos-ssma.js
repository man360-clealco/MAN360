...
(function () {
    ...
    reader.onload = async e => {
      try {
        ...
        if (tipo==='p1') { await processarP1(rows0); }
          ...
          showToastMod('Planilha não reconhecida.','erro');
  ...
  async function processarP1(rows) {
    ...
    rows.forEach(r => {
      ...
      registros.push({
        ...
        prazo, situacao:calcSituacao(prazo),
  ...
  async function processarP2(rows) {
    let n=0; const db=getDB();
    for (const r of rows) {
      const cod=String(col(r,'Código do plano de ação')||'').trim();
      if (!cod||cod==='0') continue;
      const resultado=parseFloat(col(r,'Resultado'))||0;
      const classificacao=String(col(r,'CLASSIFICAÇÃO')||col(r,'CLASSIFICAÇÃO                   DE RISCOS')||'').trim();
    ...
    finalizarImportacao(n,'Planilha 2');
  }
  ...
  function dadosFiltrados() {
    let dados = DB.filter(p=>{
      ...
      if (filtros.composicao.length) {
        ...
        const match = filtros.composicao.some(c=>{
...
.ssma-dd-btn .arr{font-size:10px;margin-left:auto;transition:transform 200ms}
...
.ssma-val-input{height:30px;width:90px;border:1px solid var(--border);border-radius:var(--radius-sm);background:var(--bg);font-family:var(--font);font-size:11px;color:#374151;padding:0 8px;text-align:right}
...
.ssma-table th .sort-ico{font-size:10px;margin-left:4px;opacity:.35}
.ssma-table th.sorted .sort-ico{opacity:1;color:var(--yellow)}
.ssma-table td{padding:10px 10px;border-bottom:1px solid var(--border);vertical-align:middle;white-space:nowrap}
.ssma-table td.desc-td{white-space:normal;min-width:200px}
...
/* Bolinhas de tratativa */
.trat-dot{display:inline-block;width:7px;height:7px;border-radius:50%;flex-shrink:0}
.trat-dot-green{background:#16a34a}
.trat-dot-red{background:#dc2626}
.trat-dot-gray{background:#d1d5db}
...
.ssma-grafico-dd-btn{height:26px;padding:0 8px;border:1px solid var(--border);border-radius:var(--radius-sm);background:var(--bg);font-family:var(--font);font-size:10px;font-weight:500;cursor:pointer;display:flex;align-items:center;gap:5px;color:#374151;white-space:nowrap}
.ssma-grafico-dd-panel{position:absolute;top:calc(100% + 3px);right:0;min-width:160px;background:var(--card-bg);border:1px solid var(--border);border-radius:var(--radius);box-shadow:var(--shadow-md);z-index:400;display:none;max-height:200px;overflow-y:auto}
.ssma-grafico-dd-panel.show{display:block}
.ssma-grafico-dd-item{display:flex;align-items:center;gap:7px;padding:6px 10px;font-size:11px;cursor:pointer;user-select:none}
.ssma-grafico-dd-item:hover{background:var(--bg)}
.ssma-grafico-dd-item input{accent-color:var(--yellow);pointer-events:none}
...
/* Tabela editável no modal */
.ssma-inline-table{width:100%;border-collapse:collapse;font-size:11px;margin-bottom:8px}
.ssma-inline-table th{text-align:left;padding:5px 7px;color:#6b7280;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.04em;border-bottom:1px solid var(--border);white-space:nowrap}
.ssma-inline-table td{padding:6px 7px;border-bottom:1px solid var(--border);vertical-align:middle}
.ssma-inline-table tr:last-child td{border-bottom:none}
.ssma-inline-table tr.editing-row td{background:#fffbeb}

.ssma-cell-input{border:1px solid var(--border);border-radius:4px;padding:4px 6px;font-size:11px;background:var(--bg);font-family:var(--font);color:#374151;width:100%;box-sizing:border-box}
...
.ssma-cell-select:focus{outline:2px solid var(--yellow);outline-offset:-1px}

/* Botões de ação inline */
.btn-icone{background:none;border:none;cursor:pointer;padding:3px 5px;border-radius:4px;font-size:13px;line-height:1;color:#6b7280;transition:background 120ms}
.btn-icone:hover{background:var(--bg)}
.btn-icone.edit:hover{color:#2563eb}
.btn-icone.del:hover{color:#dc2626}
.btn-icone.save{color:#16a34a}
.btn-icone.save:hover{background:#dcfce7}
.btn-icone.cancel:hover{color:#dc2626}
...
.ssma-hh-table tr.editing-row td{background:#fffbeb}
...
<div class="ssma">
    ...
    ${['responsavel','status','situacao','checklist','risco','classificacao'].map(n=>{
      ...
      return `<div class="ssma-dd" id="dd-${n}">
        <button class="ssma-dd-btn" id="ddbtn-${n}" onclick="ssmaToggleDD('${n}',event)">
          <i class="ti ${icons[n]}"></i><span id="ddlbl-${n}">${labels[n]}</span><i class="ti ti-chevron-down arr"></i>
        </button>
        <div class="ssma-dd-panel" id="ddp-${n}"><div id="ddl-${n}"></div></div>
      </div>`;
    }).join('')}
    <!-- Composição -->
    <div class="ssma-dd" id="dd-composicao">
      <button class="ssma-dd-btn" id="ddbtn-composicao" onclick="ssmaToggleDD('composicao',event)">
        <i class="ti ti-stack-2"></i><span id="ddlbl-composicao">Composição</span><i class="ti ti-chevron-down arr"></i>
      </button>
      <div class="ssma-dd-panel" id="ddp-composicao">
        <div id="ddl-composicao">
          ${[['somente_aq','Só aquisições'],['somente_sv','Só serviços'],['ambos','Com ambos'],['sem','Sem tratativa']].map(([v,l])=>
            `<label class="ssma-dd-item" onclick="ssmaToggleCheck('composicao','${v}',event)"><input type="checkbox" id="chk-composicao-${v}" value="${v}"> ${l}</label>`).join('')}
        </div>
      </div>
    </div>
    <!-- Modalidade de serviço -->
    <div class="ssma-dd" id="dd-modalidadeSv">
      <button class="ssma-dd-btn" id="ddbtn-modalidadeSv" onclick="ssmaToggleDD('modalidadeSv',event)">
        <i class="ti ti-tool"></i><span id="ddlbl-modalidadeSv">Modalidade</span><i class="ti ti-chevron-down arr"></i>
      </button>
      <div class="ssma-dd-panel" id="ddp-modalidadeSv"><div id="ddl-modalidadeSv"></div></div>
    </div>
  ...
  <div class="ssma-legenda">
    ...
    <span class="ssma-legenda-item"><span class="trat-dot trat-dot-green"></span>Com aquisição ou serviço registrado</span>
    <span class="ssma-legenda-item"><span class="trat-dot trat-dot-red"></span>Atrasado ou a vencer sem tratativa</span>
    <span class="ssma-legenda-item"><span class="trat-dot trat-dot-gray"></span>No prazo sem tratativa</span>
  </div>
  ...
  <div class="ssma-table-wrap">
    <table class="ssma-table">
      <thead><tr>
        <th onclick="ssmaOrdenar('codigo')" id="th-codigo">Código <span class="sort-ico">⇅</span></th>
        <th style="cursor:default">O que será feito?</th>
        <th onclick="ssmaOrdenar('prazo')" id="th-prazo">Prazo <span class="sort-ico">⇅</span></th>
        <th onclick="ssmaOrdenar('responsavel')" id="th-responsavel">Responsável <span class="sort-ico">⇅</span></th>
        <th onclick="ssmaOrdenar('vt')" id="th-vt" style="text-align:right">Valor Total <span class="sort-ico">⇅</span></th>
        <th onclick="ssmaOrdenar('risco')" id="th-risco">Risco <span class="sort-ico">⇅</span></th>
        <th onclick="ssmaOrdenar('classificacao')" id="th-classif">Classif. <span class="sort-ico">⇅</span></th>
        <th onclick="ssmaOrdenar('reclassificacao')" id="th-reclassif">Reclassif. <span class="sort-ico">⇅</span></th>
      </tr></thead>
    ...
    document.addEventListener('click', e => {
      if (!e.target.closest('.ssma-dd')&&!e.target.closest('.ssma-grafico-dd')) {
        document.querySelectorAll('.ssma-dd-panel.show,.ssma-grafico-dd-panel.show')
          .forEach(p=>p.classList.remove('show'));
        document.querySelectorAll('.ssma-dd-btn.open,.ssma-grafico-dd-btn.open')
  ...
  function popularDDs() {
    ...
    ['situacao','checklist','risco'].forEach(n=>{
      document.getElementById(`ddl-${n}`).innerHTML = fixos[n].map(v=>
        `<label class="ssma-dd-item" onclick="ssmaToggleCheck('${n}','${esc(v)}',event)"><input type="checkbox" id="chk-${n}-${esc(v)}" value="${esc(v)}"> ${esc(v)}</label>`).join('');
    });
    ...
    document.getElementById('ddl-modalidadeSv').innerHTML = modNomes.map(v=>
      `<label class="ssma-dd-item" onclick="ssmaToggleCheck('modalidadeSv','${esc(v)}',event)"><input type="checkbox" id="chk-modalidadeSv-${esc(v)}" value="${esc(v)}"> ${esc(v)}</label>`).join('');
  }

  /* Marca/desmarca sem fechar o painel */
  window.ssmaToggleCheck = function(campo, val, e) {
    e && e.stopPropagation();
    const cb = document.getElementById(`chk-${campo}-${val}`);
    if (!cb) return;
    cb.checked = !cb.checked;
    // Atualiza filtro imediatamente
    filtros[campo] = [...document.querySelectorAll(`#ddl-${campo} input:checked`)].map(x=>x.value);
    atualizarBotaoDD(campo);
    renderLista();
    if (campo==='modalidadeSv') renderGraficos();
  };
  ...
  window.ssmaLimparDD = function(n) {
    document.querySelectorAll(`#ddl-${n} input`).forEach(cb=>cb.checked=false);
    filtros[n]=[]; atualizarBotaoDD(n); renderLista();
  };
  window.ssmaAplicarDD = function(n) {
    filtros[n]=[...document.querySelectorAll(`#ddl-${n} input:checked`)].map(cb=>cb.value);
    document.getElementById(`ddp-${n}`)?.classList.remove('show');
    document.getElementById(`ddbtn-${n}`)?.classList.remove('open');
    atualizarBotaoDD(n); renderLista();
  };

  window.ssmaFiltrar = function() {
    filtros.busca=document.getElementById('ssma-busca')?.value||''; renderLista();
  };
  ...
  window.ssmaOrdenar = function(col) {
    ...
    const th=document.getElementById(`th-${col==='vt'?'vt':col==='classificacao'?'classif':col==='reclassificacao'?'reclassif':col}`);
    if (th) { th.classList.add('sorted'); const ico=th.querySelector('.sort-ico'); if(ico) ico.textContent=sortDir===1?'↑':'↓'; }
    renderLista();
  };
  ...
  function renderLista() {
    ...
    tbody.innerHTML = dados.map(p=>{
      const vt=calcValorTotal(p); const rc=p.reclassificacao||'';
      const trat=temTratativa(p);
      let dotClass='trat-dot-gray';
      if (trat) dotClass='trat-dot-green';
      else if (p.situacao==='Atrasado'||p.situacao==='A vencer') dotClass='trat-dot-red';
      const dot=`<span class="trat-dot ${dotClass}" style="margin-right:5px;vertical-align:middle"></span>`;

      let pc='prazo-g';
      if (p.situacao==='Atrasado') pc='prazo-r';
      if (p.situacao==='A vencer') pc='prazo-a';
  ...
  function renderChips() {
    ...
    if (filtros.valorMin!==null||filtros.valorMax!==null) {
      const min=filtros.valorMin!==null?fmtBRL(filtros.valorMin):'∞';
      const max=filtros.valorMax!==null?fmtBRL(filtros.valorMax):'∞';
      html+=`<span class="ssma-chip">Valor: ${min} – ${max} <button onclick="ssmaRemoverChip('valorMin','')">×</button></span>`;
    }
  ...
  function renderGraficos() {
    ...
    const ddHtml = () => {
      const modNomes=MODS.map(m=>m.nome).filter(Boolean);
      return `<div class="ssma-grafico-dd">
        <button class="ssma-grafico-dd-btn" onclick="ssmaToggleGrafDD(event)">
          <i class="ti ti-tool" style="font-size:12px"></i>
          <span id="graf-mod-lbl">${grafModFiltro.length?grafModFiltro.join(', '):'Todas modalidades'}</span>
          <i class="ti ti-chevron-down" style="font-size:10px;margin-left:4px"></i>
        </button>
        <div class="ssma-grafico-dd-panel" id="graf-mod-panel">
          ${modNomes.map(v=>`<label class="ssma-grafico-dd-item" onclick="ssmaToggleGrafMod('${esc(v)}',event)">
            <input type="checkbox" id="gchk-${esc(v)}" ${grafModFiltro.includes(v)?'checked':''}> ${esc(v)}
          </label>`).join('')}
          ${modNomes.length===0?'<div style="padding:8px 12px;font-size:11px;color:#9ca3af">Nenhuma modalidade configurada</div>':''}
        </div>
      </div>`;
    };
  ...
  function desenharPareto(canvasId, agr, fmtTick, yLabel) {
    ...
    // Cores do MAN360
    const barColors=['#F8C100','#e5ad00','#d4a000','#c49300','#b48600'];
      ...
      options:{
        ...
        plugins:{
          ...
          tooltip:{
            callbacks:{
              label(ctx2){
                if (ctx2.dataset.type==='line') return `Acumulado: ${ctx2.raw.toFixed(1)}%`;
                return `${yLabel}: ${fmtTick(ctx2.raw)}`;
              }
            }
          }
        },
  ...
  window.ssmaToggleGrafDD = function(e) {
    e.stopPropagation();
    const p=document.getElementById('graf-mod-panel');
    p?.classList.toggle('show');
  };
  window.ssmaToggleGrafDD2 = function(e) {
    e.stopPropagation();
    const p=document.getElementById('graf-mod-panel2');
    p?.classList.toggle('show');
  };
  window.ssmaToggleGrafMod = function(val, e) {
    e.stopPropagation();
    const cb=document.getElementById(`gchk-${val}`); if(!cb) return;
    cb.checked=!cb.checked;
    _sincGrafMod();
  };
  window.ssmaToggleGrafMod2 = function(val, e) {
    e.stopPropagation();
    const cb=document.getElementById(`gchk2-${val}`); if(!cb) return;
    cb.checked=!cb.checked;
    _sincGrafMod();
  };
  function _sincGrafMod() {
    grafModFiltro=[...document.querySelectorAll('[id^="gchk-"]:checked')].map(c=>c.value);
    ['graf-mod-lbl','graf-mod-lbl2'].forEach(id=>{
      const el=document.getElementById(id);
      if(el) el.textContent=grafModFiltro.length?grafModFiltro.join(', '):'Todas modalidades';
    });
    renderGraficos();
  }
  ...
  function renderModal() {
    ...
    const vt=calcValorTotal(p); const rc=p.reclassificacao||''; const classif=p.classificacao||'';
    ...
    if (modalTab==='geral') {
      ...
      bodyHtml=renderAqsView(p);
    } else if (modalTab==='servicos') {
      bodyHtml=renderSvsView(p);
    }
  ...
  /* ── Aquisições — view mode com edit/save por linha ── */
  // Estado de edição: { [i]: {campos...} }
  let aqEditando = null; // índice sendo editado, null = nenhum; -1 = nova linha

  function renderAqsView(p) {
    ...
    const rows=items.map((it,i)=>{
      if (aqEditando===i) {
        ...
        return `<tr class="editing-row">
          <td>
            <input class="ssma-cell-input" id="aq-e-cod" value="${esc(it.sem_cadastro?'':it.cod_item||'')}" ${it.sem_cadastro?'disabled':''} placeholder="Código" style="width:80px">
            <div style="display:flex;align-items:center;gap:5px;margin-top:3px;font-size:10px;color:#6b7280">
              <input type="checkbox" id="aq-e-sc" ${it.sem_cadastro?'checked':''} onchange="ssmaAqEditToggleSC()"> sem cadastro
            </div>
          </td>
          <td><input class="ssma-cell-input" id="aq-e-desc" value="${esc(it.descricao||'')}" placeholder="Descrição"></td>
          <td><select class="ssma-cell-select" id="aq-e-mod"><option value="">—</option>${MODS.map(m=>`<option value="${esc(m.nome)}" ${it.modalidade===m.nome?'selected':''}>${esc(m.nome)}</option>`).join('')}</select></td>
          <td><input class="ssma-cell-input" id="aq-e-qtd" value="${it.qtd||''}" style="width:50px;text-align:center" placeholder="0"></td>
          <td><input class="ssma-cell-input" id="aq-e-vunit" value="${it.valor_unit||''}" style="width:80px;text-align:right" placeholder="0,00"></td>
          <td style="text-align:right;font-weight:600">${fmtBRL((parseFloat(it.qtd)||0)*(parseFloat(it.valor_unit)||0))}</td>
          <td style="white-space:nowrap">
            <button class="btn-icone save" title="Salvar" onclick="ssmaAqSalvar(${i})"><i class="ti ti-check"></i></button>
            <button class="btn-icone cancel" title="Cancelar" onclick="ssmaAqCancelar()"><i class="ti ti-x"></i></button>
          </td>
        </tr>`;
      }
      return `<tr>
        ...
        <td style="text-align:right;font-weight:600">${fmtBRL((parseFloat(it.qtd)||0)*(parseFloat(it.valor_unit)||0))}</td>
        <td style="white-space:nowrap">
          <button class="btn-icone edit" title="Editar" onclick="ssmaAqEditar(${i})"><i class="ti ti-pencil"></i></button>
          <button class="btn-icone del" title="Remover" onclick="ssmaAqRemover(${i})"><i class="ti ti-trash"></i></button>
        </td>
      </tr>`;
    }).join('');

    // Linha nova (se aqEditando===-1)
    const novaLinha = aqEditando===-1 ? `<tr class="editing-row">
      <td>
        <input class="ssma-cell-input" id="aq-e-cod" placeholder="Código" style="width:80px">
        <div style="display:flex;align-items:center;gap:5px;margin-top:3px;font-size:10px;color:#6b7280">
          <input type="checkbox" id="aq-e-sc" onchange="ssmaAqEditToggleSC()"> sem cadastro
        </div>
      </td>
      <td><input class="ssma-cell-input" id="aq-e-desc" placeholder="Descrição"></td>
      <td><select class="ssma-cell-select" id="aq-e-mod"><option value="">—</option>${MODS.map(m=>`<option value="${esc(m.nome)}">${esc(m.nome)}</option>`).join('')}</select></td>
      <td><input class="ssma-cell-input" id="aq-e-qtd" style="width:50px;text-align:center" placeholder="0"></td>
      <td><input class="ssma-cell-input" id="aq-e-vunit" style="width:80px;text-align:right" placeholder="0,00"></td>
      <td>—</td>
      <td style="white-space:nowrap">
        <button class="btn-icone save" title="Salvar" onclick="ssmaAqSalvar(-1)"><i class="ti ti-check"></i></button>
        <button class="btn-icone cancel" title="Cancelar" onclick="ssmaAqCancelar()"><i class="ti ti-x"></i></button>
      </td>
    </tr>` : '';

    return `<table class="ssma-inline-table">
      <thead><tr>
        <th style="width:100px">Código</th><th>Descrição</th>
      ...
      <tbody>${rows}${novaLinha}</tbody>
    </table>
    ${aqEditando===null?`<button class="ssma-add-row" onclick="ssmaAqNovoItem()"><i class="ti ti-plus" style="font-size:12px"></i> Adicionar item</button>`:''}
    <div class="ssma-subtotal-row"><span>Subtotal aquisições</span>${fmtBRL(sub)}</div>`;
  }

  window.ssmaAqEditar    = function(i) { aqEditando=i; ssmaMudarTab('aquisicoes'); };
  window.ssmaAqCancelar  = function()  { aqEditando=null; ssmaMudarTab('aquisicoes'); };
  window.ssmaAqNovoItem  = function()  { aqEditando=-1; ssmaMudarTab('aquisicoes'); };
  window.ssmaAqEditToggleSC = function() {
    const cb=document.getElementById('aq-e-sc');
    const cod=document.getElementById('aq-e-cod');
    if (cod) { cod.disabled=cb?.checked; if(cb?.checked) cod.value=''; }
  };
  window.ssmaAqSalvar = async function(i) {
    const p=DB.find(d=>d.codigo===modalCodigo);
    const dados={
      sem_cadastro: document.getElementById('aq-e-sc')?.checked||false,
      cod_item:     document.getElementById('aq-e-cod')?.value.trim()||'',
      descricao:    document.getElementById('aq-e-desc')?.value.trim()||'',
      modalidade:   document.getElementById('aq-e-mod')?.value||'',
      qtd:          parseFloat(document.getElementById('aq-e-qtd')?.value)||0,
      valor_unit:   parseFloat(String(document.getElementById('aq-e-vunit')?.value||'').replace(',','.'))||0,
    };
    if (i===-1) {
      const {data,error}=await getDB().from('ssma_aquisicoes').insert({codigo:modalCodigo,...dados}).select();
      if (!error&&data) p._aquisicoes.push(data[0]);
    } else {
      const item=p._aquisicoes[i];
      if (item?.id) await getDB().from('ssma_aquisicoes').update(dados).eq('id',item.id);
      Object.assign(p._aquisicoes[i],dados);
    }
  ...
  /* ── Serviços — view mode com edit/save por linha ── */
  let svEditando = null;

  function renderSvsView(p) {
    ...
    const rows=items.map((it,i)=>{
      ...
      if (svEditando===i) {
        return `<tr class="editing-row">
          <td><input class="ssma-cell-input" id="sv-e-os" value="${esc(it.os||'')}" placeholder="OS" style="width:70px"></td>
          <td><input class="ssma-cell-input" id="sv-e-desc" value="${esc(it.descricao||'')}" placeholder="Descrição"></td>
          <td><select class="ssma-cell-select" id="sv-e-mod"><option value="">—</option>${MODS.map(m=>`<option value="${esc(m.nome)}" ${it.modalidade===m.nome?'selected':''}>${esc(m.nome)}</option>`).join('')}</select></td>
          <td><input class="ssma-cell-input" id="sv-e-hh" value="${it.hh_prev||''}" style="width:50px;text-align:center" placeholder="0"></td>
          ...
          <td style="white-space:nowrap">
            <button class="btn-icone save" title="Salvar" onclick="ssmaSvSalvar(${i})"><i class="ti ti-check"></i></button>
            <button class="btn-icone cancel" title="Cancelar" onclick="ssmaSvCancelar()"><i class="ti ti-x"></i></button>
          </td>
        </tr>`;
      }
      return `<tr>
        ...
        <td style="text-align:right;font-weight:600">${fmtBRL((parseFloat(it.hh_prev)||0)*taxa)}</td>
        <td style="white-space:nowrap">
          <button class="btn-icone edit" title="Editar" onclick="ssmaSvEditar(${i})"><i class="ti ti-pencil"></i></button>
          <button class="btn-icone del" title="Remover" onclick="ssmaSvRemover(${i})"><i class="ti ti-trash"></i></button>
        </td>
      </tr>`;
    }).join('');

    const novaLinha = svEditando===-1 ? `<tr class="editing-row">
      <td><input class="ssma-cell-input" id="sv-e-os" placeholder="OS" style="width:70px"></td>
      <td><input class="ssma-cell-input" id="sv-e-desc" placeholder="Descrição"></td>
      <td><select class="ssma-cell-select" id="sv-e-mod"><option value="">—</option>${MODS.map(m=>`<option value="${esc(m.nome)}">${esc(m.nome)}</option>`).join('')}</select></td>
      <td><input class="ssma-cell-input" id="sv-e-hh" style="width:50px;text-align:center" placeholder="0"></td>
      <td>—</td><td>—</td>
      <td style="white-space:nowrap">
        <button class="btn-icone save" title="Salvar" onclick="ssmaSvSalvar(-1)"><i class="ti ti-check"></i></button>
        <button class="btn-icone cancel" title="Cancelar" onclick="ssmaSvCancelar()"><i class="ti ti-x"></i></button>
      </td>
    </tr>` : '';
    ...
    return `<table class="ssma-inline-table">
      <thead><tr>
        <th style="width:78px">OS</th><th>Descrição</th>
        <th style="width:110px">Modalidade</th>
        <th style="width:58px;text-align:center">HH prev.</th>
        <th style="width:78px;text-align:right">R$/h</th>
        <th style="width:82px;text-align:right">Subtotal</th>
        <th style="width:60px"></th>
      </tr></thead>
      <tbody>${rows}${novaLinha}</tbody>
    </table>
    ${svEditando===null?`<button class="ssma-add-row" onclick="ssmaSvNovoItem()"><i class="ti ti-plus" style="font-size:12px"></i> Adicionar serviço</button>`:''}
  ...
  window.ssmaSvEditar   = function(i) { svEditando=i; ssmaMudarTab('servicos'); };
  window.ssmaSvCancelar = function()  { svEditando=null; ssmaMudarTab('servicos'); };
  window.ssmaSvNovoItem = function()  { svEditando=-1; ssmaMudarTab('servicos'); };
  window.ssmaSvSalvar   = async function(i) {
    const p=DB.find(d=>d.codigo===modalCodigo);
    const dados={
      os:        document.getElementById('sv-e-os')?.value.trim()||'',
      descricao: document.getElementById('sv-e-desc')?.value.trim()||'',
      modalidade:document.getElementById('sv-e-mod')?.value||'',
      hh_prev:   parseFloat(document.getElementById('sv-e-hh')?.value)||0,
    };
    if (i===-1) {
      const {data,error}=await getDB().from('ssma_servicos').insert({codigo:modalCodigo,...dados}).select();
      if (!error&&data) p._servicos.push(data[0]);
    } else {
      const item=p._servicos[i];
      if (item?.id) await getDB().from('ssma_servicos').update(dados).eq('id',item.id);
      Object.assign(p._servicos[i],dados);
    }
  ...
  window.ssmaFecharModal = function() {
    aqEditando=null; svEditando=null;
    const ov=document.getElementById('ssma-modal-ov'); if(ov) ov.style.display='none';
    renderLista(); renderGraficos();
  };
  ...
  function renderHH(ov) {
    const rows=MODS.map((m,i)=>{
      if (hhEditando===i) return `<tr class="editing-row">
        <td><input class="ssma-hh-input" id="hh-e-nome" value="${esc(m.nome)}" placeholder="Modalidade"></td>
        <td><input class="ssma-hh-input" id="hh-e-val" value="${m.valor_hh}" style="width:90px;text-align:right"></td>
        <td style="white-space:nowrap">
          <button class="btn-icone save" onclick="ssmaHHSalvarLinha(${i})"><i class="ti ti-check"></i></button>
          <button class="btn-icone cancel" onclick="ssmaHHCancelar()"><i class="ti ti-x"></i></button>
        </td>
      </tr>`;
      return `<tr>
        <td>${esc(m.nome)}</td>
        <td style="text-align:right">${fmtBRL(m.valor_hh)}/h</td>
        <td style="white-space:nowrap">
          <button class="btn-icone edit" onclick="ssmaHHEditar(${i})"><i class="ti ti-pencil"></i></button>
          <button class="btn-icone del" onclick="ssmaHHRemover(${i})"><i class="ti ti-trash"></i></button>
        </td>
      </tr>`;
    }).join('');
    const novaLinha = hhEditando===-1 ? `<tr class="editing-row">
      <td><input class="ssma-hh-input" id="hh-e-nome" placeholder="Nome da modalidade"></td>
      <td><input class="ssma-hh-input" id="hh-e-val" style="width:90px;text-align:right" placeholder="0"></td>
      <td style="white-space:nowrap">
        <button class="btn-icone save" onclick="ssmaHHSalvarLinha(-1)"><i class="ti ti-check"></i></button>
        <button class="btn-icone cancel" onclick="ssmaHHCancelar()"><i class="ti ti-x"></i></button>
      </td>
    </tr>` : '';
    ...
    ov.innerHTML=`<div class="ssma-modal" style="max-width:420px">
      ...
      <div class="ssma-modal-body">
        <table class="ssma-hh-table">
          <thead><tr><th>Modalidade</th><th style="text-align:right">R$/h</th><th style="width:60px"></th></tr></thead>
          <tbody>${rows}${novaLinha}</tbody>
        </table>
        ${hhEditando===null?`<button class="ssma-add-row" onclick="ssmaHHNovoItem()"><i class="ti ti-plus" style="font-size:12px"></i> Nova modalidade</button>`:''}
      </div>
    </div>`;
    ov.style.display='flex';
  }
  ...
  window.ssmaHHEditar      = function(i){hhEditando=i;renderHH(document.getElementById('ssma-hh-ov'));};
  window.ssmaHHCancelar    = function(){hhEditando=null;renderHH(document.getElementById('ssma-hh-ov'));};
  window.ssmaHHNovoItem    = function(){hhEditando=-1;renderHH(document.getElementById('ssma-hh-ov'));};
  window.ssmaHHRemover     = async function(i){
    ...
    MODS.splice(i,1); hhEditando=null; renderHH(document.getElementById('ssma-hh-ov'));
    popularDDs();
  };
  window.ssmaHHSalvarLinha = async function(i){
    const nome=document.getElementById('hh-e-nome')?.value.trim()||'';
    const val=parseFloat(document.getElementById('hh-e-val')?.value)||0;
    if (!nome){showToastMod('Nome obrigatório','erro');return;}
    if (i===-1){
      const {data,error}=await getDB().from('ssma_modalidades').insert({nome,valor_hh:val}).select();
      if(!error&&data) MODS.push(data[0]);
    } else {
      const m=MODS[i];
      if(m.id) await getDB().from('ssma_modalidades').update({nome,valor_hh:val}).eq('id',m.id);
      MODS[i]={...m,nome,valor_hh:val};
    }
    hhEditando=null; renderHH(document.getElementById('ssma-hh-ov'));
    popularDDs(); showToastMod('Modalidade salva','ok');
  };
